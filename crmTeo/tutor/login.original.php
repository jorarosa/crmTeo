...
if ($acc == 'rclave'){
	...
	if (strlen($user) > 0 ){
		...
		$r=mysql_query($q) or die ("Correo no hallado");
		$row=mysql_fetch_array($r);	
		if ($row){$clave=$row[0];
			$asunto="Acceso a su cuenta";

			$mensaje="La clave para acceder a su cuenta";
			$mensaje.="en la página de FCT del IES CLARA DEL REY  es \gn\n'".$clave."'\n\n";
			$mensaje.="Saludos.";
...
if ($acc == 'login') {
	...
	if(strlen($user) == 0 || strlen($clave)==0 ){
		...
		$q="select clave from $TABLA_LOGIN where email='$user'";	
		$r=mysql_query($q) or die ("Correo o clave err&oacute;neas");
		$row=mysql_fetch_array($r);	
		$clave_hallada=$row[0];
...
?>
	...
	<tr>
	<td> <img src=../content/tutor.jpg> </td>
	<td align=center>
